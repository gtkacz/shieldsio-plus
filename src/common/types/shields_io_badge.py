...
from src.common.types import HexCode, ShieldsIOColor
...
class ShieldsIOBadge:
	...
	def __parse_shields_io_color_object(color_obj: Optional[ShieldsIOColor]) -> str:
		if color_obj:
			if isinstance(color_obj, (ShieldsIONamedColors, HexCode)):
	...
	def build_shieldsio_badge_str(self) -> str:
		badge_content = "-".join(filter(None, [self.label, self.message, self.color]))

		query = {
			"style": self.style.value,
			"labelColor": self.__label_color,
			"color": self.__color,
			"logo": self.build_base64_logo(),
		}
		...
		return f"{badge_content}?{query_string}"
